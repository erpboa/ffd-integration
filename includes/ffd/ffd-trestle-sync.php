...
class FFD_Trestle_Sync {
    ...
    public $token_refresh_try=0;//verify ????
    ...
    /**
     * Main FFD_Trestle_Sync Instance.
     *
     * Ensures only one instance of FFD_Trestle_Sync is loaded or can be loaded.
     *
     * @since 1.0.0
     * @static
     * @see FFD()
     * @return FFD_Trestle_Sync - Main instance.
     */
    ...
    public function init_hooks(){
        ...
        add_action('ffd_trestletowp_credentials', array($this, 'credentials_updated'), 10, 1);

        //$wptopb = get_option('ffd_wptotrestle_sync', 'no');//sync trestle to wp
        $pbtowp = get_option('ffd_trestletowp_sync', 'no');
        /*if( 'yes' === $wptopb &&  $pbtowp !== 'yes' ){ //sync trestle to wp
            add_action('ffd_wptotrestle_sync', array($this, 'sync_to_pb'));
        }*/
        /*add_action('init', function(){ //sync trestle to wp
    ...
    public function plugin_activated(){
        ...
        if( !$timestamp ){
            $timestamp = wp_schedule_event(time(), 'hourly', 'ffd_wptotrestle_sync');
        }
    ...
    public function plugin_deactivated(){
        ...
        $timestamp = wp_next_scheduled( 'ffd_main_sync');
        if( $timestamp ){
            wp_unschedule_event( $timestamp, 'ffd_wptotrestle_sync');
        }

        update_option('_wptotrestle_synced', '');
    }

    /*
    * When credentials are updated in
    * trestle to wp api calls ( specificaly tokens )
    */
    public function credentials_updated($data){
        ...
        do_action('ffd_logger', array('wptotrestle_credentials_set' => 'by trestletowp'));
    }
    ...
    public function sync_to_pb(){
        ...
        $log_file = $upload_dir['basedir'] . '/wptotrestle_' . date('Y-m-d') . '.log';
        ...
        $synced = get_option('_wptotrestle_synced', array());
        if(empty($synced) )
            $synced = array();
        ...
        if( $posts ){
            foreach($posts as $post ){
                ...
                if( $listing_id ){
                    ...
                    update_option('_wptotrestle_synced', $synced);
    ...
    public function get_posts_to_sync($args=array()){
        ...
        $defaults = array(
            ...
            'sync_to_pb' => 1,
            'fields' => 'ids',
            'ffd_wpquery_label' => 'wptotrestle_posts'
        );
        ...
        $args = apply_filters('ffd_wptotrestle_postargs', $args);
    ...
    public function get_post_data($post_id){
        ...
        wp_reset_postdata();
        return apply_filters('ffd_wptotrestle_postdata', $formatted_data, $this);
    ...
    public function get_media_data($post){
        ...
        $data = apply_filters('ffd_wptotrestle_mediadata', $data, $this);
    ...
    public function send_request( array $args=array()){
        ...
        if( empty(get_option('ffd_trestle_access_token') )){
        ...
        if (  isset($response->error) && $response->error == "unsupported_grant_type") {
            ...
            if( 1 > $this->token_refresh_try  && $this->refresh_token() ){
                ...
                if( 1 > $this->token_refresh_try ){
                    ...
                    update_option('ffd_trestle_access_token', '');
                }
    ...
    public  function update_credentials($data){
        ...
        foreach( $this->credentials_names as $name ){
            if( isset($data[$name]) ){
                update_option('ffd_trestle_' . $name, $data[$name]);
                $this->credentials[$name] = $data[$name];
                $updated++;
            }
        }
    ...
    public function get_mappings($context="default"){
        ...
        $fields_mapping = get_option( 'trestle'. '_fields_mapping', array());
...
/**
 * Main instance of FFD_Trestle_Sync.
 *
 * Returns the main instance of FFD_UI to prevent the need to use globals.
 *
 * @since  1.0
 * @return FFD_Trestle_Sync
 */
function FFD_Trestle() {
    return FFD_Trestle_Sync::instance();
}
FFD_Trestle();
