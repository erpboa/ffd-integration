 ...
 *
 * @class    FFD_Listings_Trestle_Sync
 * @author   FrozenFish
 * @category Admin
...
/**
 * FFD_Listings_Trestle_Sync class.
 */
class FFD_Listings_Trestle_Sync extends FFD_Trestle_API {
    ...
    public static function run(){
        ...
        $pbtowp = get_option('ffd_trestletowp_sync', 'no');
        //$wptopb = get_option('ffd_wptotrestle_sync', 'no');
        ...
        if( 'yes' === $pbtowp /*&&  $wptopb !== 'yes'*/ ){
            ...
            add_action('ffd_trestletowp_sync_status', array(__CLASS__, 'check_sync_status'));
        }
    ...
    public static function set_sync_interval($interval){
        ...
        $sync_interval = get_option('ffd_trestle_sync_interval', 'hourly');
    ...
    public static function test(){
        ...
        $records = self::test_trestle_query();
    ...
    * wp to trestle api calls ( specificaly tokens )
    */
    public function credentials_updated($data){
        ...
        do_action('ffd_logger', array('trestletowp_credentials_set' => 'by wptotrestle'));
    }
    ...
    public static function activated(){
        ...
        wp_schedule_event(time(), 'twicedaily', 'ffd_trestletowp_sync_status');
    ...
    public static function de_activated(){
        ...
        $timestamp = wp_next_scheduled( 'ffd_trestletowp_sync_status');
        if( $timestamp ){
            wp_unschedule_event( $timestamp, 'ffd_trestletowp_sync_status');
        }
        ...
        update_option('ffd_trestle_sync_ids', '');
        update_option('ffd_trestle_sync_status', 'idle');
        update_option('ffd_trestle_sync_index', '0');
    ...
    public static function run_main_sync(){
        ...
        $file1 = $back_trace[0]['file'];
        $file2 = $back_trace[1]['file'];
        $line1 = $back_trace[0]['line'];
        $line2 = $back_trace[1]['line'];
        $function1 = $back_trace[0]['function'];
        $function2 = $back_trace[1]['function'];
        $argum1 = $back_trace[0]['args'][0];
        $argum2 = $back_trace[1]['args'][0];

        do_action('ffd_logger', array('[FFD Rum Main Sync 2020]' => $pbtowp.' started trestle to livemodern array 0 '.$file1.' '.$line1.' '.$function1.' '.$argum1.' array 1 '.$file2.' '.$line2.' '.$function2.' '.$argum2));
    ...
    public static function reschedule_main_sync(){
        ...
        //var_dump('reschedule_main_sync');exit;
        return $scheduled;
    }
    ...
    public static function start_sync(){
        ...
        //do_action('ffd_logger', array('[FFD Sync]' => self::$sync_type . ' started trestle to livemodern array 0 '.$file1.' '.$line1.' '.$function1.' array 1 '.$file2.' '.$line2.' '.$function2));
        ...
        //do_action('ffd_logger', array('[FFD Fields Querys]' => self::$fields));
        ...
        if( !empty( self::$fields ) ){
            ...
            //implementar para borrar closed
    ...
    public static function prune_listings(){
        ...
        if( !empty($query) ){
            ...
            while (!$done) {
                ...
                $results = self::trestle_get_listings();
    ...
    public static function import_listings(){
        ...
        do_action('ffd_logger', array('[FFD Sync]' => self::$sync_type . ' importing !!!!!'.self::$query));

        self::$methodUrl = "/trestle/odata/Property" . self::$query;
        ...
        self::$did_timeout = self::start_importing(); //==================================================================0
    ...
    public static function start_importing(){
        ...
        while (!self::$sync_done && !self::$did_timeout ) {
            ...
            //self::$listings = self::trestle_get_listings();//==================================================================0
            //self::$listings->totalSize = count((self::$listings)->value);
            self::$total_records = isset(self::$listings->totalSize) ? self::$listings->totalSize : null;
           // do_action('ffd_logger', array('[FFD Total Records Sync]' => 'Total Listing Size: '.self::$total_records));
           //do_action('ffd_logger', array('[FFD LISTINGS Sync]' => self::$listings));
            /*if( self::$debugger ){
            ...
            //do_action('ffd_logger', array('[FFD Total Records Sync FUERA 2020]' => 'Total Listing Size: '.self::$total_records.'----'.self::$listings->totalSize));
            if (isset(self::$listings->totalSize) && self::$listings->totalSize>0 ) {

                //self::$sync_done = self::$listings->done;
                self::$sync_done = isnull(self::$listings)?false:true;
                //Revisar
                /*if (!self::$sync_done) {
                    self::$methodUrl = self::$listings->nextRecordsUrl;
                }*/
                //do_action('ffd_logger', array('[FFD Total Records Sync DENTRO 2020]' => 'Total Listing Size: '.self::$total_records.'----'.self::$listings->totalSize));
                //self::upsert_listings();==================================================================0
    ...
    public static function upsert_listings(){
        ...
        $looped = 0;
        foreach ((self::$listings)->value as $property) {
            try {

                if ( $property && isset($property->ListingKey) ) {
                    ...
                    //foreach ($property->pba__Listings__r->records as $record) {
                        ...
                        $ffd_pb_record = $property;

                        self::$media = isset($property->Media) ? $property->Media : array();
    ...
    private static function current_record_mls_id(){
        ...
        $mls_id =  isset(self::$record->ListingKey) ? self::$record->ListingKey : null;
        if( empty($mls_id) )
            $mls_id =  isset(self::$record->ListingId) ? self::$record->ListingId : null;
    ...
    private static function current_record_salesforce_id(){
        ...
        $salesforce_id = isset(self::$record->ListingKey) ? self::$record->ListingKey : ( isset(self::$record->ListingId) ? self::$record->ListingId : null );
    ...
    public static function create_listing(){
        ...
        $addr = self::$record['StreetNumber'].' '.self::$record['StreetDirPrefix'].' '.self::$record['StreetName'].' #'.self::$record['UnitNumber'];
        $address = ucwords($addr.' '.self::$record['City'].', '.self::$record['StateOrProvince'].' '.self::$record['PostalCode']);

        $city =  isset(self::$record['City'])?self::$record['City'].'-':'-' ;
        if($city != '-')
            $city =  str_replace(' ','-', $city);

        $street =  isset(self::$record['StreetName'])?self::$record['StreetName'].'-':'-' ;
        if($street != '-')
            $street =  str_replace(' ','-', $city);

        $addr_guid = (isset(self::$record['StreetNumber']) && self::$record['StreetNumber']!=''?self::$record['StreetNumber'].'-':'').
            (isset(self::$record['StreetDirPrefix']) && self::$record['StreetDirPrefix']!=''?self::$record['StreetDirPrefix'].'-':'').
            $street.
            (isset(self::$record['UnitNumber']) && self::$record['UnitNumber']!=''?self::$record['UnitNumber'].'-':'').
            $city.
            (isset(self::$record['StateOrProvince']) && self::$record['StateOrProvince']!=''?self::$record['StateOrProvince'].'-':'').
            (isset(self::$record['PostalCode'])?self::$record['PostalCode']:'');
        $addr_guid = strtolower($addr_guid);
        ...
        $application = isset($_SERVER['CONTEXT_PREFIX'])?$_SERVER['CONTEXT_PREFIX']:'twocityplazacondos';
        $guid = $protocol . '://' . $server_name . '/' . $application . '/listing/' . $addr_guid . '/';*/

        /*begin franklin.espinoza*/

        $post_title = isset($address) ? $address : '';
        $post_name = isset($addr_guid) ? $addr_guid : '';
        $post_content = isset(self::$record['PublicRemarks']) ? self::$record['PublicRemarks'] : '';
    ...
    */
    ...
    public static function get_record_images(){
        ...
        if ( isset(self::$media) ){
            foreach (self::$media as $media) {
                array_push($images, $media['MediaURL']);
            }
        } else {
            return false;
        }
    ...
    public static function add_listing_images($images){
        ...
        $media_field_key = get_option('ffd_trestle_media_field', 'ffd_media');
    ...
    */
    ...
    protected static function get_post_id_by_mls_id($mls_id=null){
        ...
        $post_id = $wpdb->get_var("SELECT a.ID as post_id 
                                   FROM $wpdb->posts a 
                                   LEFT JOIN $wpdb->postmeta b ON a.ID=b.post_id 
                                   WHERE a.post_type='$posttype' AND b.meta_key='ffd_id' AND b.meta_value='".$mls_id."';");
        if( !is_null($post_id) )
            return $post_id;
    ...
    protected static function get_post_id_by_salesforce_id($salesforce_id=null){
        ...
        $post_id = $wpdb->get_var("SELECT a.ID as post_id 
                                    FROM $wpdb->posts a 
                                    LEFT JOIN $wpdb->postmeta b ON a.ID=b.post_id 
                                    WHERE a.post_type='$posttype' AND b.meta_key='ffd_salesforce_id' AND b.meta_value='".$salesforce_id."';");
        if( !is_null($post_id) )
            return $post_id;
    ...
    public static function sync_init(){
        ...
        self::$post_fields = self::fields(/*null*/'trestle', 'post_fields');

        self::$fields = self::fields(/*null*/'trestle', 'sync');
    ...
    public static function fields($platform=null, $context='view'){
        ...
        $trestle_default_fields = self::trestle_api_default_fields();

        //do_action('ffd_logger', array('[Fields Mapping ---->2023]' => $fields_mapping));
        ...
        if( empty($fields) ){
            foreach($fields_mapping as $field_name => $field_settings ){
                if( isset($trestle_default_fields[strtolower($field_name)]) ){
                    $fields[$field_name] = 'ffd_' . strtolower(trim(str_replace(array('pba__', '__c'), '', $field_name), '_'));
                }
            }
        }
        //do_action('ffd_logger', array('[Fields ---->2022]' => $fields));
        if($context=='sync'){
            //do_action('ffd_logger', array('[Platform ---->2022]' => $platform));
            //do_action('ffd_logger', array('[Table ---->2022]' => $post_table));
            //do_action('ffd_logger', array('[Fields ---->2022]' => $fields));
        }

        //Revisar para manejar con Trestle
        if( $context == 'meta' ){
    ...
    public static function query(){
        ...
        self::$default_mapped = apply_filters('ffd_sync_default_mapped', array(
            'post_title'          => 'StreetName',
            'post_name'           => 'StreetName',
            'post_content'        => 'PublicRemarks',
            'post_status'         => 'StandardStatus'/*,
        ...
        if(get_option('trestle_expand')){$filter = '$expand='.get_option('trestle_expand'); $param[]=$filter;}
    ...
    /*public static function where(){
        ...
        do_action('ffd_logger', array('[Debugg Mapping ---->2023]' => $lastRun.'>---<'.$where));
    ...
    protected static function trestle_get_listings(){

        //do_action('ffd_logger', array('[FFD Get Listing]' => 'Error: pb get_listings manually disabled.'));
        do_action('ffd_logger', array('[FFD Get Listing]' =>  self::$api_settings['instance_url'].'<---->'.self::$methodUrl.'<--->'.self::$api_settings['access_token']));
        ...
        //do_action('ffd_logger', array('[FFD Trestle API Request Listing Sync 2020]' =>  $json ));
    ...
    public static function test_trestle_query(){
        ...
        $json = self::trestle_get_listings();
    ...
    protected static function memory_exceeded() {
        ...
        return apply_filters( 'ffd_trestle_api_memory_exceeded', $return );
    }
    ...
    protected static function time_exceeded() {
        ...
        $finish = self::$start_time + apply_filters( 'ffd_trestle_api_default_time_limit', self::$timeout_limit );
        $return = false;
        if ( time() >= $finish ) {
            $return = true;
        }
        return apply_filters( 'ffd_trestle_api_time_exceeded', $return );
    }
