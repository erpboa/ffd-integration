...
class FFD_Template_Rendrer {
	...
	function rb_display_listings($atts, $content=null){//var_dump('rb_display_listings');exit;
 ...
 /*--------------------------------------------------------------------------------------
	...
	*/
	function get_post_data($post_id, $context='single'){
		...
		$post = get_post($post_id);//var_export($post);exit;
		setup_postdata($post);
		...
		foreach($meta as $key => $value){
			...
			if( !empty($value) && is_string($value) ){
				if( apply_filters('ffd_get_post_data', true, $key, $value) ){//var_dump($key, $value);exit;
                    //var_dump($key, $value, ffd_get_field($key, $post_id));exit;
//apply format
                    if($key == 'ffd_list_price_sqft' || $key == 'ffd_taxes' || $key == 'ffd_living_sq_ft'){
                        $data['meta'][$key] = number_format(maybe_unserialize(ffd_get_field($key, $post_id)), 0, '.', ',');
                    }else {
                        $data['meta'][$key] = ffd_get_field($key, $post_id);
                    }

				    if ($key == 'ffd_media'){
                        $data['meta'][$key] =  explode(',',trim(str_replace('"','',$data['meta'][$key]),'{}'));
                        //var_dump($data['meta'][$key]);exit;
                    }
		...
		if( $post->post_type === 'listing' ){
			...
			foreach($pb_fields as $name => $key ){ //var_dump($name , $key); var_dump('====================');
				if( isset($data['meta'][$key]) && !empty($data['meta'][$key]) ){
					if( $name === 'image' ){

						$ffd_media = $data['meta'][$key];

                        $ffd_media = (explode(',',trim(str_replace('"','',$ffd_media),'{}')))[0];//var_dump( $ffd_media,json_encode($ffd_media), json_encode(json_decode($ffd_media))) ;
    ...
    function get_post_data_trestle($post_id, $context='single'){
...
//                    $data['meta'][$key] = ffd_get_field($key, $post_id);
                    if($key == 'ffd_list_price_sqft' || $key == 'ffd_taxes' || $key == 'ffd_living_sq_ft'){
                        $data['meta'][$key] = number_format(ffd_get_field($key, $post_id), 0, '.', ',');
                    }else {
                        $data['meta'][$key] = ffd_get_field($key, $post_id);
                    }
                }
...
function FFD_Template_Rendrer() {
	return FFD_Template_Rendrer::instance();
}
//if( !(is_admin() && !wp_doing_ajax()) ){
	FFD_Template_Rendrer();
//}
...
function ffd_build_wpquery_args($atts){
	...
	$params = array(
		...
		$current_data = FFD_Template_Rendrer()->get_post_data($post->ID);
